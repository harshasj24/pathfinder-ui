...
export class ItRunSpendComponent implements OnInit {
  ...
  handleClick() {
    ...
    if (project) {
      let { itrunspend } = project;
      this.val = itrunspend;
      console.log(itrunspend);
      let inflation: any = {};
      let clientShare: any = {};
      let yearlyRunRate: any = {};
      itrunspend?.runCalc.map((element: any, index: number) => {
        if (element.itSpendWithInflation) {
          inflation['year' + (index + 1)] = element.itSpendWithInflation;
          inflation.baseline = itrunspend.baseLineYearSpend;
          inflation.total = itrunspend.total_itSpendWithInflation;
          inflation.title = 'IT Spend with Inflation';
        }
        if (element.cotClientShare) {
          clientShare['year' + (index + 1)] = element.cotClientShare;
          clientShare.total = itrunspend.total_cotClientShare;
        ...
        if (element.yearlyRunRate) {
          ...
          yearlyRunRate.total = itrunspend.total_yearlyRunRate;
